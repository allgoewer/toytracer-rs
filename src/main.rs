...
pub fn to_ppm<W: io::Write>(w: &mut W, values: impl Iterator<Item=Color>, width: usize, height: usize) -> io::Result<()> {
    writeln!(w, "P3")?;
...
fn main() -> std::io::Result<()> {
    ...
    let img = (0..image_height).rev().map(move |j| {
    ...
    to_ppm(&mut stdout.lock(), img, image_width, image_height)?;
